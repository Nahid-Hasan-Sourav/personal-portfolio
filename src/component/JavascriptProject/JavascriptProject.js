...
const JavascriptProject = () => {
    const [JavascriptProject,setjavascriptProject]=useState(null)
    useEffect(()=>{
        fetch('allprodetails.json')
        .then(res=>res.json())
        .then(data=>setjavascriptProject(data))
      },[])
    return (
        <>
        <div className='grid lg:grid-cols-3 gap-6 mt-6'>
            {
                JavascriptProject?.filter((item)=>{
                    return(
                        item.projectCategory==="JAVASCRIPT"
